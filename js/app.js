...
const viewProductDetails = (id) => {
    showSpinner('single_product');
    loadSingleProduct(id);
}

// Show spinner
const showSpinner = (id) => {
    const modalBody = document.getElementById(id);
    modalBody.innerHTML = '';
...
// Show Single Product on Modal 

const showSingleProduct = (product) => {
    const modalBody = document.getElementById('single_product');
    modalBody.innerHTML = '';
    const elem = `
        <div class="modal-header">
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body single-product border-boxshadow-none text-center" >
            <div>
        ...
        <div class="modal-footer">
            ...
            <button type="button" onclick="addToCart(${product.id},${product.price})" class="btn buy-now">Add to Cart</button>
        </div>
    `
    modalBody.innerHTML = elem;
}

// Buy Now 
// This function removed cart data and show user a message
const buyNow = () => {
    if (getInputValue("total") > 0) {
        document.getElementById('buy_now_text').innerText = 'Your Order is Submitted';
        ...
        // Cart is empty
        document.getElementById('buy_now_text').innerText = 'Add Some Product to your Cart';
    }
...
// Search Function 
// This function will run when someone click the search button
const search = () => {
    ...
    // Add search input field placeholder value
    document.getElementById('input-field').placeholder = 'Search Products by Name, Category';

}

// Filtering search products 
// This function filter search products by search text
const filterSearchedProducts = (searchText, data) => {
    // filter
    filteredProducts = data.filter(product => product.title.toLowerCase().includes(searchText.toLowerCase()) || product.category.toLowerCase().includes(searchText.toLowerCase()));
...
const loadAllProducts = (searchText) => {
    const URL = 'https://fakestoreapi.com/products';
    fetch(URL)
        .then(res => res.json())
        .then(data => filterSearchedProducts(searchText, data));
}

// Clear previous products
const clearProducts = () => {
    // set inner text of all-products div to empty
...
const showProducts = (products) => {
    // Check is there products available or not
    // If product available then show all the product in the UI
    // Else show 'No Product Found Message in search box
    if (products.length > 0) {
        ...
        for (const product of allProducts) {
            ...
            <button onclick="viewProductDetails(${product.id})" class="details btn" data-bs-toggle="modal" data-bs-target="#productModal">Details</button>
            </div>
            </div>
            `;
        ...
        document.getElementById('input-field').placeholder = 'No products found! Try another name...';
    }
